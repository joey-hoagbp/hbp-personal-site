...
- **Content & i18n:** language-neutral data (links, tech names, project metadata) is in `app/data.ts`; all translatable copy is in `app/i18n/dictionary.ts` keyed by locale (`vi`/`en`). Language state is a client Context in `app/i18n/LanguageProvider.tsx`; components read it via `useLang()`. Default language is **Vietnamese**, persisted to `localStorage` (`"lang"`); the switcher is `LangToggle.tsx`, a two-pill VI/EN control rendered by `SiteNav.tsx` (`Nav.tsx` no longer exists). It stays visible in the nav bar at every width, and a second copy renders at the foot of `MobileNavSheet.tsx` for reachability while the sheet is open below 768px (the four nav links, the theme toggle and the Contact CTA collapse into that sheet below 768px — four links plus both toggles no longer fit one bar).
...
- Sections on `/`: Hero · ProofRow · Stack (`#skills`) · Work (`#portfolio`) · Experience (`#experience`) · Education (`#education`) · Contact. Experience and Education are two sections over one shared row style — the `.tl-*` timeline grid in `globals.css` (date / role / optional note); education rows carry no note, so their fourth column stays empty.
...
There is **no test harness** in the frontend; verification is `npm run build` + `npm run lint` + manual browser check. **Stop every running `next dev` before running the build.** `next dev` and `next build` share `.next/`, so a build run while a dev server is up replaces the running server's chunks: it then throws `MODULE_NOT_FOUND`, every asset 404s and the page reloads with no CSS. Recovery is stop the dev servers, `rm -rf frontend/.next`, restart. Splitting the two with a phase-dependent `distDir` was tried and reverted — with `output: "export"` it scatters the build across two directories and leaves `out/` stale, which would ship a stale site to Pages. When running the backend on a non-default port, set `NEXT_PUBLIC_API_BASE_URL` in `frontend/.env.local` to match.
